...
import { initNetwork, sendNetMessage, triggerPanic, authAdmin } from './network-v1';
...
function App() {
  ...
  const doAuth = async () => {
    ...
    try {
      ...
      store.setProfile({ seed: data.seed, currentId: data.gid, displayName: data.name, publicKeyJwk: '', privateKey: null, publicKey: null, isOwner: data.isOwner });
    ...
    <div onClick={() => setShowProfile(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 400, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '8px 20px 20px' }}>
          ...
          <button onClick={saveProfile} style={{ width: '100%', padding: 12, borderRadius: 8, background: 'var(--accent)', color: 'var(--bg)', fontSize: 14, fontWeight: 600, border: 'none', cursor: 'pointer' }}>
            {t('save')}
          </button>
        </div>
      </div>
  ...
  const ctxMenuEl = contextMenu && (
    <div style={{ position: 'fixed', left: contextMenu.x, top: contextMenu.y, zIndex: 2000, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 10, padding: 4, minWidth: 160, boxShadow: '0 8px 24px rgba(0,0,0,0.15)' }}>
      {[
        { label: t('profile'), action: () => { setSelectedChat(contextMenu.id); /* profile opens from chat header */ } },
        { label: t('clearChat'), action: () => store.clearMessages(contextMenu.id) },
        { label: store.getContact(contextMenu.id)?.blocked 
          ...
          action: () => { const c = store.getContact(contextMenu.id); if (c?.blocked) store.unblockContact(contextMenu.id); else store.blockContact(contextMenu.id); }
        },
  ...
  if (adminMode) {
    return (
      <div style={{ height: '100vh', width: '100vw', background: '#0a0000', color: '#fff', display: 'flex', flexDirection: 'column' }}>
        {fileInputs}
        {profileModal}
        {ctxMenuEl}
        <div style={{ padding: '12px 20px', borderBottom: '2px solid #ff000040', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#110000' }}>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          <div style={{ width: 300, borderRight: '1px solid #ff000020', overflowY: 'auto', padding: 16 }}>
            <div style={{ fontSize: 11, color: '#ff444480', marginBottom: 12, fontWeight: 700 }}>{t('users')} ({adminUsers.length})</div>
            {adminUsers.map((u, i) => (
              <button key={i} onClick={() => { if (!store.getContact(u.id)) store.addContact(u.id, { displayName: u.a, currentId: u.id, publicKey: '', lastSeen: Date.now() }); setSelectedChat(u.id); }}
                style={{ width: '100%', padding: '10px 12px', marginBottom: 6, borderRadius: 8, background: selectedChat === u.id ? '#ff000015' : '#ffffff05', border: selectedChat === u.id ? '1px solid #ff000040' : '1px solid #ffffff10', cursor: 'pointer', textAlign: 'left', color: '#fff' }}>
                <div style={{ fontSize: 13, fontWeight: 500 }}>{u.a}</div>
                <div style={{ fontSize: 9, color: '#888', fontFamily: 'monospace' }}>ID: {u.id}</div>
                <div style={{ fontSize: 9, color: '#666' }}>IP: {u.ip}</div>
              </button>
            ))}
            {adminUsers.length === 0 && <div style={{ color: '#444', fontSize: 12, textAlign: 'center', padding: 20 }}>No users</div>}
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid #ff000020' }}>
              <button onClick={() => setSelectedChat('AnoAI_bot')} style={{ width: '100%', padding: '10px 12px', borderRadius: 8, background: '#ffffff05', border: '1px solid #ffffff10', cursor: 'pointer', textAlign: 'left', color: '#fff', fontSize: 12 }}>AnoAI Requests</button>
            </div>
          </div>
            ...
            {selectedChat && cc ? (
              ...
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#333' }}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 28, fontWeight: 800, color: '#ff4444', letterSpacing: '0.15em', marginBottom: 8 }}>ADMIN PANEL</div>
                  <div style={{ fontSize: 12, color: '#555' }}>Select a user</div>
                </div>
              </div>
            )}
          </div>
    ...
    <div style={{ height: '100vh', width: '100vw', display: 'flex', background: 'var(--bg)' }}>
      ...
      <div style={{ width: 320, flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border)', height: '100%' }}>
        ...
        <ChatList contacts={contacts} selectedChat={selectedChat} onSelectChat={handleSelectChat}
          onAddContact={id => store.addContact(id, { displayName: id, currentId: id, publicKey: '', lastSeen: Date.now() })}
